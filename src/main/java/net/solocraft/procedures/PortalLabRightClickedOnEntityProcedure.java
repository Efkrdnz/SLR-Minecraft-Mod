...
public class PortalLabRightClickedOnEntityProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if (!MagicReadingHelper.isHoldingMagicReader(sourceentity)) {
			...
			final ResourceKey<Level> dungeonDimension = ResourceKey.create(
					...
					ResourceLocation.parse("sololeveling:dungeon_dimension_a"));
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator.getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse("shadows")))
			...
			SololevelingMod.queueServerWork(10, () -> {
				...
				net.solocraft.util.UrgentQuestManager.markDungeonId(sourceentity, "lab");
				SololevelingMod.queueServerWork(5, () -> {
					...
					SololevelingMod.queueServerWork(10, () -> {
						if (!PlayerEntryGenerationGuard.isCurrent(entryPlayer,
								...
								|| entryPlayer.level().dimension() != dungeonDimension)
							return;
						if ((entity instanceof PortalLabEntity _datEntL26 && _datEntL26.getEntityData().get(PortalLabEntity.DATA_usedbefore)) == false) {
							if (entity instanceof PortalLabEntity _datEntSetL)
								_datEntSetL.getEntityData().set(PortalLabEntity.DATA_usedbefore, true);
							{
								Entity _ent = sourceentity;
								if (!_ent.level().isClientSide() && _ent.getServer() != null) {
									_ent.getServer().getCommands()
											.performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4,
													...
													"execute in sololeveling:dungeon_dimension_a as @s at @s unless entity @e[type=sololeveling:portal_12,distance=..100] run spawnlab");
								}
							}
							sourceentity.getPersistentData().putString("dungeon_tag", (entity.getStringUUID()));
						}
			...
			MagicReadingHelper.showRankReading(sourceentity, ProceduralDungeonRank.A);
		}
	}
}
