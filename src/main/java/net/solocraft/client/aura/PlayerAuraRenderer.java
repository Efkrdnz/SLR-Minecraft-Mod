...
/**
 * Renders reusable aura geometry directly in the player render pass. No aura
 * entities are spawned, and the already-rendered player depth keeps rear aura
 * layers from washing over the model.
 */
...
public final class PlayerAuraRenderer {
	...
	public static void renderPlayerAura(RenderPlayerEvent.Post event) {
		...
		if (player.isSpectator() || minecraft.level == null)
			return;
		// PlayerRenderer normally is not called for the local first-person body,
		// but keep this explicit so camera/body mods cannot fill the user's view.
	...
	private static void renderFluidTrail(RenderPlayerEvent.Post event, PlayerAuraDefinition definition,
			AuraInstance instance, Vec3 currentPosition, long gameTime, Minecraft minecraft) {
		...
		for (int i = 1; i < trail.size(); i++) {
			...
			drawTrailEcho(vertices, poseStack, definition, radius, height,
					event.getEntity().tickCount + partialTick - age, fade, i, minecraft);
			poseStack.popPose();
		}
	}
	...
	private static void renderInstance(RenderPlayerEvent.Post event, PlayerAuraDefinition definition,
		...
		if (instance.duration() >= 0) {
			float progress = Math.min(1.0F, (gameTime - instance.startTick() + partialTick) / instance.duration());
		...
		for (int layer = 0; layer < definition.shellLayers(); layer++) {
			...
			int alpha = alpha((34.0F - layer * 7.0F) * envelope);
			drawShell(vertices, poseStack, definition, layerRadius, height * (0.96F + layer * 0.04F), rotation, alpha);
		}
	...
	private static void drawShell(VertexConsumer vertices, PoseStack poseStack, PlayerAuraDefinition definition,
			float radius, float height, float rotationDegrees, int alpha) {
		poseStack.pushPose();
	...
	private static void drawWisps(VertexConsumer vertices, PoseStack poseStack, PlayerAuraDefinition definition,
			int seed, float radius, float height, float motion, float envelope, Minecraft minecraft) {
		for (int i = 0; i < definition.wispCount(); i++) {
			...
			int alpha = alpha((72.0F + hash(seed + i * 13) * 52.0F) * envelope * (0.55F + 0.45F * wave(phase * (float) Math.PI)));
			drawTaperedQuad(vertices, poseStack.last(), width, wispHeight, definition, alpha, 2.0F);
			if (definition.facing() == PlayerAuraDefinition.Facing.CROSSED) {
				poseStack.mulPose(Axis.YP.rotationDegrees(90.0F));
				drawTaperedQuad(vertices, poseStack.last(), width, wispHeight, definition, alpha / 2, 2.0F);
			}
			poseStack.popPose();
		}
	}

	private static void drawSpikes(VertexConsumer vertices, PoseStack poseStack, PlayerAuraDefinition definition,
			int seed, float radius, float height, float motion, float envelope, Minecraft minecraft) {
		for (int i = 0; i < definition.spikeCount(); i++) {
			...
			poseStack.pushPose();
			poseStack.translate((float) Math.sin(Math.toRadians(angle)) * orbit, y,
					(float) Math.cos(Math.toRadians(angle)) * orbit);
			applyFacing(poseStack, definition.facing(), angle, minecraft);
			poseStack.mulPose(Axis.ZP.rotationDegrees(-18.0F + hash(seed + i) * 36.0F));
			drawDiamond(vertices, poseStack.last(), width, spikeHeight, definition,
					alpha((105.0F + pulse * 90.0F) * envelope));
			poseStack.popPose();
		}
	}

	private static void drawFluidField(VertexConsumer vertices, PoseStack poseStack,
			PlayerAuraDefinition definition, Player player, float radius, float height, float motion,
			float envelope, float partialTick, Minecraft minecraft) {
		...
		drawFluidBackflow(vertices, poseStack, definition, fluid, player, radius, height,
	...
	private static void drawTrailEcho(VertexConsumer vertices, PoseStack poseStack,
		...
		for (int i = 0; i < 4; i++) {
			...
			float y = height * (0.10F + i * 0.19F)
					+ Mth.sin(motion * 0.04F + i * 1.9F) * height * 0.035F;
	...
	private static void drawFluidLobes(VertexConsumer vertices, PoseStack poseStack,
		...
		for (int i = 0; i < fluid.lobeCount(); i++) {
			...
			float life = fract(i * 0.618034F + motion * fluid.speed() * (0.0065F + (i % 4) * 0.00055F));
			float lifeFade = (float) (Mth.smoothstep(Math.min(1.0F, life * 6.0F))
					* Mth.smoothstep(Math.min(1.0F, (1.0F - life) * 5.0F)));
			float orbit = radius * fluid.radiusScale() * (0.70F + random * 0.24F)
					* (1.0F + Mth.sin(motion * 0.035F + i * 1.41F) * 0.045F * fluid.turbulence());
	...
	private static void drawFluidBackflow(VertexConsumer vertices, PoseStack poseStack,
			PlayerAuraDefinition definition, PlayerAuraDefinition.FluidProfile fluid, Player player,
			float radius, float height, float motion, float envelope, float partialTick, Minecraft minecraft) {
		...
		for (int i = 0; i < fluid.backflowCount(); i++) {
			...
			float lateral = (i / (float) Math.max(1, fluid.backflowCount() - 1) - 0.5F) * radius * 1.05F;
			...
			float width = radius * (0.20F + hash(i * 37 + 13) * 0.15F);
	...
	private static void drawFluidQuad(VertexConsumer vertices, PoseStack.Pose pose, float width,
			float height, PlayerAuraDefinition definition, int alpha, float uvBase) {
		int bottomColor = mixColor(definition.secondaryColor(), definition.primaryColor(), 0.20F);
		int topColor = mixColor(definition.primaryColor(), 0xFFFFFF, 0.18F);
	...
	private static void drawSoftBackflow(VertexConsumer vertices, PoseStack.Pose pose,
	...
	private static void drawGroundRing(VertexConsumer vertices, PoseStack poseStack, PlayerAuraDefinition definition,
			float radius, float motion, int alpha) {
		...
		for (int segment = 0; segment < RING_SEGMENTS; segment++) {
			...
			double a0 = Math.PI * 2.0D * t0;
			double a1 = Math.PI * 2.0D * t1;
			vertex(vertices, poseStack.last(), (float) Math.cos(a0) * inner, 0.025F, (float) Math.sin(a0) * inner, 6.0F + t0, 1.0F, definition.secondaryColor(), alpha / 2);
			vertex(vertices, poseStack.last(), (float) Math.cos(a1) * inner, 0.025F, (float) Math.sin(a1) * inner, 6.0F + t1, 1.0F, definition.secondaryColor(), alpha / 2);
	...
	private static void applyFacing(PoseStack poseStack, PlayerAuraDefinition.Facing facing,
			float crossedAngle, Minecraft minecraft) {
	...
	private static void drawTaperedQuad(VertexConsumer vertices, PoseStack.Pose pose, float width, float height,
			PlayerAuraDefinition definition, int alpha, float uvBase) {
	...
	private static void drawDiamond(VertexConsumer vertices, PoseStack.Pose pose, float width, float height,
			PlayerAuraDefinition definition, int alpha) {
	...
	private static void vertex(VertexConsumer vertices, PoseStack.Pose pose, float x, float y, float z,
		...
		vertices.vertex(matrix, x, y, z)
				.color((color >> 16) & 255, (color >> 8) & 255, color & 255, Math.max(0, Math.min(255, alpha)))
	...
	private static int mixColor(int first, int second, float amount) {
		...
		int red = Math.round(Mth.lerp(t, (first >> 16) & 255, (second >> 16) & 255));
		int green = Math.round(Mth.lerp(t, (first >> 8) & 255, (second >> 8) & 255));
		int blue = Math.round(Mth.lerp(t, first & 255, second & 255));
