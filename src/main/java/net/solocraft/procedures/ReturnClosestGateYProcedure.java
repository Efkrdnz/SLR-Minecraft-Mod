...
public class ReturnClosestGateYProcedure {
	public static String execute(LevelAccessor world, double x, double y, double z) {
		if (!world.getEntitiesOfClass(PathfinderMob.class, AABB.ofSize(new Vec3(x, y, z), 200, 200, 200), e -> true).isEmpty()) {
			...
			}.compareDistOf(x, y, z)).findFirst().orElse(null)).getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse("portals")))) {
