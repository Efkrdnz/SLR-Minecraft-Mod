		...
		if (!((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == SololevelingModItems.MAGIC_READER.get())) {
			...
			SololevelingMod.queueServerWork(10, () -> {
				if (sourceentity instanceof ServerPlayer _player && !_player.level().isClientSide()) {
					ResourceKey<Level> destinationType = ResourceKey.create(Registries.DIMENSION, new ResourceLocation("sololeveling:dungeon_dimension_d"));
				...
				net.solocraft.util.UrgentQuestManager.markDungeonId(sourceentity, "goblin_sewers");
				SololevelingMod.queueServerWork(5, () -> {
					...
					sourceentity.getPersistentData().putString("dungeon_tag", (entity.getStringUUID()));
					SololevelingMod.queueServerWork(10, () -> {
						if ((entity instanceof PortalSewersEntity _datEntL26 && _datEntL26.getEntityData().get(PortalSewersEntity.DATA_usedbefore)) == false) {
							if (entity instanceof PortalSewersEntity _datEntSetL)
								_datEntSetL.getEntityData().set(PortalSewersEntity.DATA_usedbefore, true);
							{
								Entity _ent = sourceentity;
								if (!_ent.level().isClientSide() && _ent.getServer() != null) {
									_ent.getServer().getCommands()
											.performPrefixedCommand(
													...
													"execute in sololeveling:dungeon_dimension_d as @s at @s unless entity @e[type=sololeveling:portal_12,distance=..100] run spawnrandom");
								}
							}
							sourceentity.getPersistentData().putString("dungeon_tag", (entity.getStringUUID()));
						}
