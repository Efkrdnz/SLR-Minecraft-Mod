					...
					if (CooldownManager.getRemainingTicks(_owner, "Blessing Mark") % 20 == 0) {
						if (world instanceof Level _level) {
							if (!_level.isClientSide()) {
								_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("sololeveling:bellirng")), SoundSource.NEUTRAL, (float) 0.1, 1);
							} else {
								_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("sololeveling:bellirng")), SoundSource.NEUTRAL, (float) 0.1, 1, false);
							}
						}
					}
				} else {
