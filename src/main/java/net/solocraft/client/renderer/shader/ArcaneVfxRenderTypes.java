...
public final class ArcaneVfxRenderTypes extends RenderStateShard {
	private static ShaderInstance shader;
	private static RenderType cachedEffect;

	private ArcaneVfxRenderTypes(String name, Runnable setup, Runnable clear) {
	...
	public static void registerShaders(RegisterShadersEvent event) throws IOException {
		event.registerShader(new ShaderInstance(event.getResourceProvider(),
				ResourceLocation.fromNamespaceAndPath(SololevelingMod.MODID, "rendertype_arcane_vfx"),
	...
	public static RenderType effect(ResourceLocation fallback) {
		...
		if (cachedEffect == null) {
			...
			cachedEffect = RenderType.create("arcane_vfx", WorldShaderVertexFormat.NEW_ENTITY,
					VertexFormat.Mode.QUADS, 8192, false, true, state);
		}
		return cachedEffect;
	}
}
