...
public class RandomCaveLargeRightClickedOnEntityProcedure {
		...
		if (!MagicReadingHelper.isHoldingMagicReader(sourceentity)) {
			...
			SololevelingMod.queueServerWork(10, () -> {
				if (sourceentity instanceof ServerPlayer _player && !_player.level().isClientSide()) {
					ResourceKey<Level> destinationType = ResourceKey.create(Registries.DIMENSION, ResourceLocation.parse("sololeveling:dungeon_dimension_c"));
				...
				SololevelingMod.queueServerWork(5, () -> {
					...
					sourceentity.getPersistentData().putString("dungeon_tag", (entity.getStringUUID()));
					SololevelingMod.queueServerWork(10, () -> {
						if ((entity instanceof RandomCaveLargeEntity _datEntL26 && _datEntL26.getEntityData().get(RandomCaveLargeEntity.DATA_usedbefore)) == false) {
							if (entity instanceof RandomCaveLargeEntity _datEntSetL)
								_datEntSetL.getEntityData().set(RandomCaveLargeEntity.DATA_usedbefore, true);
							{
								Entity _ent = sourceentity;
								if (!_ent.level().isClientSide() && _ent.getServer() != null) {
									_ent.getServer().getCommands()
											.performPrefixedCommand(
													...
													"execute in sololeveling:dungeon_dimension_c as @s at @s unless entity @e[type=sololeveling:portal_12,distance=..100] run spawnrandomlarge");
								}
							}
							sourceentity.getPersistentData().putString("dungeon_tag", (entity.getStringUUID()));
						}
