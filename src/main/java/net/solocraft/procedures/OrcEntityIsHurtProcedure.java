...
public class OrcEntityIsHurtProcedure {
	public static void execute(LevelAccessor world, Entity entity, Entity sourceentity) {
		...
		double rand = 0;
		rand = Math.random();
		if (!(sourceentity instanceof Player _plr ? _plr.getAbilities().instabuild : false) && !(sourceentity instanceof OrcEntity)) {
			if (rand >= 0.85) {
				...
				SololevelingMod.queueServerWork(25, () -> {
					{
						...
						for (Entity entityiterator : _entfound) {
							if (!(entity == entityiterator) && !(entityiterator instanceof OrcEntity)) {
								entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.MOB_ATTACK_NO_AGGRO), entity), 12);
