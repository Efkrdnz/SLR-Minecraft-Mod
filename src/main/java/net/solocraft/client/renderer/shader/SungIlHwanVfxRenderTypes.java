...
/**
 * Depth-tested surface and additive submissions for Sung Il-Hwan's black,
 * silver, and pale-gold spatial effects.
 */
@EventBusSubscriber(
...
public final class SungIlHwanVfxRenderTypes extends RenderStateShard {
	...
	private static final RenderType SURFACE = create(
			"sung_il_hwan_vfx_surface", () -> surfaceShader,
			TRANSLUCENT_TRANSPARENCY);
	private static final RenderType EMISSIVE = create(
			"sung_il_hwan_vfx_emissive", () -> emissiveShader,
			ADDITIVE_TRANSPARENCY);

	private SungIlHwanVfxRenderTypes(String name, Runnable setup,
			Runnable clear) {
		...
		try {
			event.registerShader(new ShaderInstance(event.getResourceProvider(),
					ResourceLocation.fromNamespaceAndPath(SololevelingMod.MODID,
							"rendertype_sung_il_hwan_vfx_surface"),
					WorldShaderVertexFormat.NEW_ENTITY),
					loaded -> surfaceShader = loaded);
		} catch (IOException exception) {
			LOGGER.warn("[SoloLeveling] Sung Il-Hwan surface shader did not load; using the vanilla fallback.",
					exception);
		}
		try {
			event.registerShader(new ShaderInstance(event.getResourceProvider(),
					ResourceLocation.fromNamespaceAndPath(SololevelingMod.MODID,
							"rendertype_sung_il_hwan_vfx_emissive"),
					WorldShaderVertexFormat.NEW_ENTITY),
					loaded -> emissiveShader = loaded);
		} catch (IOException exception) {
			LOGGER.warn("[SoloLeveling] Sung Il-Hwan emissive shader did not load; using the vanilla fallback.",
					exception);
		}
	}
