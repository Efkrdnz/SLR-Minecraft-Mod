						...
						hei = hei + 0.133;
						{
							...
							if (!_ent.level().isClientSide() && _ent.getServer() != null) {
								_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4,
										_ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), ("/particle dust 0.61 0 0 1 " + x_pos + " " + (y_pos + 1.8) + " " + z_pos + " 0 0 0 0.1 1 force"));
							}
						}
			...
			if (sourceentity.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("dm")))) {
				if (sourceentity.getPersistentData().getDouble("Level") > 0) {
					if (Math.random() < (5 * rnk) / ((float) sourceentity.getPersistentData().getDouble("Level"))) {
						...
						if (world instanceof Level _level) {
							if (!_level.isClientSide()) {
								_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("sololeveling:swordclash")), SoundSource.NEUTRAL, (float) 0.3, (float) 1.2);
							} else {
								_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("sololeveling:swordclash")), SoundSource.NEUTRAL, (float) 0.3, (float) 1.2, false);
							}
						}
					}
				}
