		...
		if (!((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == SololevelingModItems.MAGIC_READER.get())) {
			...
			SololevelingMod.queueServerWork(10, () -> {
				if (sourceentity instanceof ServerPlayer _player && !_player.level().isClientSide()) {
					ResourceKey<Level> destinationType = ResourceKey.create(Registries.DIMENSION, new ResourceLocation("sololeveling:dungeon_dimension_b"));
				...
				net.solocraft.util.UrgentQuestManager.markDungeonId(sourceentity, "lush");
				SololevelingMod.queueServerWork(5, () -> {
					...
					sourceentity.getPersistentData().putString("dungeon_tag", (entity.getStringUUID()));
					SololevelingMod.queueServerWork(10, () -> {
						if ((entity instanceof PortalLushEntity _datEntL26 && _datEntL26.getEntityData().get(PortalLushEntity.DATA_usedbefore)) == false) {
			...
			rand = Mth.nextInt(RandomSource.create(), 601, 799);
